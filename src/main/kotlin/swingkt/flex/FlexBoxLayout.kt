...
private data class RelativeDimension(val mainAxis: Int, val crossAxis: Int)
...
class FlexBoxLayout(
        ...
        parent?.let { target ->
            synchronized (target.getTreeLock()) {
                ...
                components.forEachIndexed { index, component ->
                    val r = map[component]
                    if (r != null) {
                        val pos = RelativeDimension(r.mainAxisPos, r.crossAxisPos).toNormal()
                        val size = RelativeDimension(r.mainAxisSize, r.crossAxisSize).toNormal()
    ...
    private fun RelativeDimension.toNormal(): Dimension {
        return if (flexDirection == FlexDirection.ROW) Dimension(mainAxis, crossAxis) else Dimension(crossAxis, mainAxis)
    }

    override fun preferredLayoutSize(parent: Container?): Dimension {
        var size: Dimension
        val normalized = RelativeDimension(mainAxisTotal, crossAxisTotal).toNormal()
        synchronized(this) {
            calculate(parent!!)
