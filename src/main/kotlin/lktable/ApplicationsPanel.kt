...
class ApplicationsMainPanel(private val state: AppState) : FlexBoxPanel() {
    ...
    init {
        ...
        flexrow(alignItem = FlexAlignItem.MAX) {
...
class ApplicationPanel(private val application: Application, private val state: AppState) : FlexBoxPanel(FlexBoxLayout(gap = 4, alignItem = FlexAlignItem.CENTER)) {
...
fun Container.hoverIcon(icon: Icon, hoverIcon: Icon, hovered: (Boolean) -> Unit = {}, block: HoverIconButton.() -> Unit = {}) = component(HoverIconButton(icon, hoverIcon, hovered), block)
